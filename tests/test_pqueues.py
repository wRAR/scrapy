...
class TestPriorityQueue:
    ...
    def test_queue_push_pop_one(self, crawler: Crawler) -> None:
        temp_dir = tempfile.mkdtemp()
        queue = ScrapyPriorityQueue.from_crawler(crawler, FifoMemoryQueue, temp_dir)
    ...
    def test_no_peek_raises(self, crawler: Crawler) -> None:
    ...
    def test_peek(self, crawler: Crawler) -> None:
    ...
    def test_queue_push_pop_priorities(self, crawler: Crawler) -> None:
        temp_dir = tempfile.mkdtemp()
        queue = ScrapyPriorityQueue.from_crawler(
            crawler, FifoMemoryQueue, temp_dir, [-1, -2, -3]
        )
        assert queue.pop() is None
        assert len(queue) == 0
        req1 = Request("https://example.org/1", priority=1)
...
class TestDownloaderAwarePriorityQueue:
    @pytest.fixture
    def queue(self) -> Generator[DownloaderAwarePriorityQueue]:
    ...
    def test_no_peek_raises(self, queue: DownloaderAwarePriorityQueue) -> None:
        if hasattr(queuelib.queue.FifoMemoryQueue, "peek"):
            pytest.skip("queuelib.queue.FifoMemoryQueue.peek is defined")
        queue.push(Request("https://example.org"))
        with pytest.raises(
