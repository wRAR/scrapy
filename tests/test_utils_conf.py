...
class BuildComponentListTest(unittest.TestCase):
    ...
    def test_duplicate_components_in_dict(self):
        ...
        self.assertRaises(ValueError, build_component_list, {}, duplicate_dict, convert=lambda x: x.lower())

    def test_duplicate_components_in_list(self):
        duplicate_list = ['a', 'b', 'a']
        self.assertRaises(ValueError, build_component_list, None, duplicate_list, convert=lambda x: x)
    ...
    def test_duplicate_components_in_basesettings(self):
        ...
        duplicate_bs.set('ONE', duplicate_bs['ONE'], priority=20)
        self.assertRaises(ValueError, build_component_list, duplicate_bs, convert=lambda x: x.lower())
...
class FeedExportConfigTestCase(unittest.TestCase):
    ...
    def test_feed_export_config_backward_compatible(self):
        with warnings.catch_warnings(record=True) as cw:
            settings = Settings()
            self.assertEqual(
                {'items.dat': {'format': 'csv'}}, feed_process_params_from_cli(settings, ['items.dat'], 'csv')
            )
    ...
    def test_feed_complete_default_values_from_settings_non_empty(self):
        ...
        self.assertEqual(
            new_feed,
            {"encoding": "other encoding", "fields": None, "indent": 42, "store_empty": True, "uri_params": None,},
        )
