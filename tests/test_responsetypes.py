...
def test_from_filename(source: str, expected: type[Response]) -> None:
    retcls = responsetypes.from_filename(source)
    assert retcls is expected
...
def test_from_content_disposition(
    source: str | bytes, expected: type[Response]
) -> None:
...
def test_from_content_type(source: str, expected: type[Response]) -> None:
...
def test_from_body(source: bytes, expected: type[Response]) -> None:
...
def test_from_headers(source: dict[str, list[str]], expected: type[Response]) -> None:
    headers = Headers(source)
    retcls = responsetypes.from_headers(headers)
    assert retcls is expected
...
def test_from_args(source: dict[str, Any], expected: type[Response]) -> None:
