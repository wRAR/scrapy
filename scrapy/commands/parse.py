...
class Command(BaseRunSpiderCommand):
    ...
    def add_options(self, parser):
        ...
        parser.add_option(
            "-m", "--meta", dest="meta", help="inject extra meta into the Request, it must be a valid raw json string"
        )
        parser.add_option(
            "--cbkwargs",
            dest="cbkwargs",
    ...
    def get_callback_from_rules(self, spider, response):
        if getattr(spider, 'rules', None):
            ...
            logger.error(
                'No CrawlSpider rules found in spider %(spider)r, ' 'please specify a callback to use for parsing',
                {'spider': spider.name},
            )
